...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: { canonical: `/moduller/${auditModule.slug}` },
  };
}
...
export default async function ModuleDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!criteria) {
    ...
    throw new Error(`Module ${auditModule.code} has no transcribed criteria in lib/criteria/.`);
  }
        ...
        <div className="grid gap-10 lg:grid-cols-[1fr_auto] lg:items-start">
          ...
          <Reveal delay={0.06}>
            <div className="flex flex-col gap-4 rounded-xl2 border border-line bg-bg-soft p-6 lg:w-72">
              ...
              <p className="text-xs leading-relaxed text-ink-muted">{SCALE_NOTE}</p>
            </div>
          </Reveal>
        </div>
      </section>
            ...
            {bounds.map((criterion, index) => (
              <Reveal key={criterion.code} delay={Math.min(index, 6) * 0.03}>
                <li className="flex items-baseline gap-3 rounded-xl2 border border-line bg-surface px-4 py-3">
                  ...
                  <span className="font-mono text-sm text-accent-strong">{criterion.threshold}</span>
                </li>
              </Reveal>
            ))}
          </ul>
