      ...
      {hasCorporateIdentity() ? null : (
        <Card tone="accent" className="border-2">
          ...
          <p className="mt-3 text-base leading-relaxed text-ink">
            ...
            dairesi/numarası bu metinde henüz yer almamaktadır. Bu bilgiler resmî sicil kayıtlarıdır;
            uydurulmamış, boş bırakılmıştır. Metin, satış yoluna açılmadan önce gerçek sicil
...
export default async function IptalIadePage({ params }: { params: Promise<LocaleParams> }) {
  ...
  return (
    <LegalPage title="İptal & İade Politikası" updated="16 Temmuz 2026">
      <LegalSection title="1. Kapsam">
        <p>
          Bu politika, {siteConfig.url} adresi üzerinden kart ile satın alınan denetim ve eğitim
          hizmetlerinin iptali ile bedel iadesine ilişkin koşulları düzenler.{" "}
          <LegalLink href="/mesafeli-satis-sozlesmesi">Mesafeli Satış Sözleşmesi</LegalLink> ve{" "}
          <LegalLink href="/on-bilgilendirme">Ön Bilgilendirme Formu</LegalLink> ile birlikte
          uygulanır ve bu metinlerin ayrılmaz parçasıdır.
        </p>
        <p>
          Teklif yoluyla ilerleyen ve karttan tahsilat yapılmayan taleplerde bu politika uygulanmaz;
          bu tür işlerin koşulları tarafların imzaladığı ayrı sözleşmede belirlenir.
        </p>
      </LegalSection>

      <LegalSection title="2. İptalin Dayandığı Ölçüt: Denetim Tarihi">
        <p>
          Bu hizmette iade koşullarını belirleyen şey, siparişin üzerinden geçen süre değil{" "}
          <strong>denetim ziyaretine kalan süredir</strong>. Sebebi somuttur: satın alma anında
          satıcı tarafında bir maliyet doğmaz, ancak ziyaret takvime alındığı anda bir denetçi o
          tarihe ayrılır, tesiste konaklama rezerve edilir ve seyahat planlanır. Ziyaret tarihi
      ...
      <LegalSection title="3. İptal Basamakları">
        <Card tone="accent" className="border-2">
          <Eyebrow>Oranlar taslaktır — işletme sahibi tarafından belirlenecek</Eyebrow>
          <p className="mt-3 text-base leading-relaxed text-ink">
            Aşağıdaki gün aralıkları ve kesinti oranları bir <strong>öneridir</strong>, karar
            değildir. Yayına almadan önce işletme sahibi tarafından gerçek denetçi ve seyahat
            maliyetlerine göre belirlenmeli, hukuk danışmanınca teyit edilmelidir. Fiili zararı aşan
            bir kesinti, alıcı tarafından itiraza açıktır.
          </p>
        </Card>
        <LegalList items={cancellationLadder} />
        <p>
          Kesinti oranı, siparişin tamamı üzerinden değil, iptal edilen hizmet kalemleri üzerinden
          hesaplanır. Kısmi iptal hâlinde iptal edilmeyen kalemler yürürlükte kalır.
        </p>
      </LegalSection>

      <LegalSection title="4. Ziyaret Başladıktan Sonra İade Yapılmaz">
        <p>
          Denetçinin tesise misafir sıfatıyla giriş (check-in) yapmasıyla hizmetin ifası başlamış
          sayılır. <strong>Bu andan itibaren iptal ve iade mümkün değildir.</strong>
        </p>
        <p>
          Bunun sebebi hizmetin doğasıdır: ziyaret başladığı anda ölçüm yapılmış, denetçi kaynağı ve
          seyahat maliyeti harcanmış olur ve yapılan gözlem geri alınamaz. Mesafeli Sözleşmeler
          Yönetmeliği&apos;nin 15. maddesi de, alıcının onayı ile ifasına başlanan hizmetlerde cayma
          hakkının kullanılamayacağını düzenler. Alıcı, ödeme adımında ifanın cayma süresi içinde
          başlamasına açıkça onay verir.
        </p>
        <p>
          Raporun bulgularından memnun olunmaması bir iade sebebi değildir; denetim, sonucu önceden
          taahhüt edilen bir hizmet değil, ölçüm hizmetidir. Ancak raporun sipariş edilen modülleri
          kapsamaması, kriterlerin eksik ölçülmesi veya bulguların kanıta bağlanmaması hâlinde alıcı
          ayıplı ifa hükümlerine başvurabilir; bu durumda öncelikle ziyaret ücretsiz olarak
          yenilenir.
        </p>
      </LegalSection>
      ...
      <LegalSection title="5. Erteleme — İptale Alternatif">
        <p>
          ...
          <strong>7 günden fazla süre</strong> kala talep edilen ilk erteleme kesintisiz yapılır ve
          yeni tarih taraflarca mutabakatla belirlenir. Sonraki erteleme talepleri, o an geçerli olan
          iptal basamağına göre değerlendirilir.
        </p>
        <p>
          Erteleme süresi de bu bölümde bir taslaktır; işletme sahibi tarafından belirlenecektir.
        </p>
      </LegalSection>

      <LegalSection title="6. İade Süresi ve Yöntemi">
        <p>
          İade, <strong>ödemenin yapıldığı kartın kendisine</strong> yapılır. Nakit, havale veya
          başka bir hesaba iade yapılamaz; bu, kart kuruluşlarının kuralıdır ve satıcının tercihi
          değildir.
        </p>
        <LegalList
          items={[
      ...
      <LegalSection title="7. Mücbir Sebep">
        <p>
          Aşağıdaki hâllerden birinin ziyaretin gerçekleştirilmesini engellemesi durumunda hiçbir
          taraf temerrüde düşmüş sayılmaz ve iptal basamakları uygulanmaz:
        </p>
        <LegalList items={forceMajeureEvents} />
        <p>
          Mücbir sebep hâlinde ziyaret öncelikle kesintisiz olarak ertelenir. Engelin ortadan
          kalkmaması veya taraflarca yeni bir tarihte mutabık kalınamaması hâlinde sözleşme sona erer
          ve tahsil edilen tutarın tamamı, hiçbir kesinti yapılmaksızın 6. bölümdeki usulle iade
          edilir.
        </p>
        <p>
          Mücbir sebep, ziyaret başladıktan sonra ortaya çıkarsa hizmetin ifa edilen kısmı bakımından
          iade yapılmaz; ifa edilemeyen kısım için taraflar mutabakatla çözüm belirler.
        </p>
      </LegalSection>
      ...
      <LegalSection title="8. Satıcı Bilgileri ve Başvuru">
        <SellerIdentity />
        <p>
          İptal, iade ve erteleme talepleriniz ile bu politikaya ilişkin itirazlarınızı{" "}
          <LegalLink href={`mailto:${siteConfig.contact.email}`}>
            {siteConfig.contact.email}
          </LegalLink>{" "}
          ...
          <LegalLink href="/mesafeli-satis-sozlesmesi">Mesafeli Satış Sözleşmesi</LegalLink>&apos;nin
